...
- 단계
  - 1단계 인트로 
  - 2단계 개인정보 입력 /개인정보수집동의(보험분석 필수동의, 상품/서비스소개선택동의, 가입설계 선택동의)  
  - 3단계 휴대폰 본인인증 
  - 4단계 진단결과 제공
##### 고객용 보험진단 화면 
- 진단 결과 카카오 알림톡으로 전송
- 보험진단 : 종합점수, 각 진단그룹별 보장현황(뇌혈관진환, 실손의료비, 사망, 치아/골절/화상, 심장질환, 수술비, 후유장해, 운전/주택, 암, 입원일당)
- 진단상세 : 각 진단그릅별 상세 보장별 권장금액/현황 제공. 추천문구/추천상품(Admin 연동)
- 상담신청 : 전화상담 신청 (2단계에서 선택 미동의 시 필수항목으로 동의 팝업 활성화)
...
- 커튼배너 : 착한보험연구소 메뉴 외 최상단 노출되는 배너 영역. 1개~2개로 구성. 상품 추천 및 이벤트 안내, 시스템 공지 시 사용 (Admin 배너 관리)
    - 커튼배너 이미지형 : 주로 상품 또는 서비스 추전 시 사용
    ...
    - 커튼배너 등록 시 이미지 배경 컬러 색상 코드 등록 필수
...
### 착한보험 안내서
- 블로그성 컨텐츠, 웹툰 컨텐츠로 보험 상품을 안내하는 메뉴. 현재 웹툰형 컨텐츠를 주로 제공하고 있음
- 월 1~2회 업데이트 및 기존 심의번호 수정 작업 진행 (22년07월 이후 업데이트 없음)
- Admin 메뉴 : 다이렉트랩 > 작한보험 안내서
  ...
  * 업무순서
    ...
    2. 배너 문구 현업에 컨펌 요청 
    3. 디자인 진행 
    4. 1차 개발계 Admin DB 등록 
    5. 개발계 현업 컨펌 요청 
    6. 2차 운영계 Admin DB 등록
  * 목록 화면 하단 outlink 용 배너 2개 노출 : Admin > 직판 배너 > PC미들 작한보험안내
### 기타
####  멤버십 '멜다방' 
- 현재 운영하지 않음 2022년 7월 기준
- 별도 가입한 멤버십 회원 대상으로 월별 이벤트 제공 (스크래치이벤트, 룰렛이벤트, 선물상자고르기 이벤트 등)
- 매월 말 또는 초 이벤트 오픈. 이벤트 오픈 시 멜다방 메인, 이벤트 목록 외 직판 모바일 메인배너 노출. 친구톡 배너 노출 진행됨
...
#### 이벤트
- 네이버결제 혜택, 신용카드 결제 혜택 매월 말일 업데이트 진행 (휴일/공휴일이 말일인 경우 마지막 영업일에 배포 진행)
- 결제 혜택 외 이벤트 진행 시 당첨자발표 이미지 등록 (PC만)
...
- 배너 관리 : 중앙배너, 상품추천배너, 기타 우측배너 Admin 관리. 
  * 그 외 모두 코딩 작업
  * Admin 등록 및 코딩 시 웹접근성을 위한 배너 AIt 에시지 등록 필수
- 재방문 Re-Marketing 배너 영역 : 쿠키 정보 이용, 최근 방문한 상품 재노출 영역 (Admin 배너관리)
### 인터넷고객센터
- 현재 운영팀 업무범위 아님, 
- 비정기적으로 여의되T팀에서 별도 요정이 와서 업무 진행 함
- 계약 관리 전용 메뉴
### 보상/보험금 청구
- 보험금 청구 전용 메뉴
...
- 공시실 (공시 관련은 DATA 및 Admin 연결되어 있으나, 하위 메뉴는 업데이트 작업 필요)
  * 상품공시 > 보험가격공시 중 일반손해보험의 경우 퍼블 작업
  * 모집수수료율 > 금융기관보험대리점 모집수수료율: 퍼블 작업 (월 1회)
  * 가사도우미 지원비용, 간병인/재택간병인 지원비용: 퍼블 작업 (비정기적)
- 소비자보호광장, FOREIGN : 각 담당 부서에서 수정 요청 시 업무 진행.
- 사이트맵
  * 상품 신규 및 메뉴 신규/변경 시 사이트맵 동기화 작업 필요 (개발에서 진행)
  * 메뉴명 업데이트 : 개발
  * 메뉴 링크 : 개발
### 웹접근성 인증 갱신 작업
- 인증 업체: 웹와치
- 매년 2월 갱신 (1월 초부터 1개월 동안 검수 및 접근성 위배 내용 수정 반영, 직판은 진행하지 않음, 대표 홈페이지만 진행)
- 웹접근성 위배 기능 중 아래 기능은 심사 기간 동안 비노출 치리 후 심사 완료 후 재반영
...
- 각 상품별, 위치 영역별 PC/MO 배너 디자인 작업
- 등록 Admin 다이렉트 플랫폼 > 스마트오퍼 > 이미지관리
- 대표PC 장기체류팝업 : 특정 시간 동안 메뉴 이동 없는 경우 자동 활성화되는 팝업
### 통합설문
- 설문용 랜딩. 설문 스킨은 1개로 구성. 각 설문별 코드로 기간별 제공 진행
- Admin > 기타관리 > 설문관리 (현업)
### 퀴즈랜딩
- 퀴즈 이벤트용 랜딩. 퀴즈 템플릿 5개로 구성 (건강, 상식, 주관식) 추천상품별
- Admin > 다이렉트플랫폼 > 퀴즈관리 (현업)
### TM소호재용랜딩
